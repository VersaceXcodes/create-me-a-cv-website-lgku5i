...
export default tool({
  ...
  execute: async ({ featureId, message }) => {
    const response = await fetch("https://launchpulse.ai/api/project/create-me-a-cv-website/features/progress", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        projectId: "create-me-a-cv-website",
        token: "lp_createme_65fe418a75364129",
        featureId: featureId,
        passes: true,
