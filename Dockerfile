...
RUN sed -i 's|VITE_BACKEND_URL=.*|VITE_BACKEND_URL=https://create-me-a-cv-website-lgku5i.launchpulse.ai|g' .env 2>/dev/null || true
RUN echo 'VITE_API_BASE_URL=https://create-me-a-cv-website-lgku5i.launchpulse.ai' > .env.production.local && echo 'VITE_BACKEND_URL=https://create-me-a-cv-website-lgku5i.launchpulse.ai' >> .env.production.local
RUN npm run build
...
# Copy frontend build output to public directory (Vite outputs to dist by default)
